...
class LedControllerThread(Thread):
    ...
    def run(self):
        ...
        while not self._stop_event.wait(0.01):
            if self._settings.get("screen.ready") == "true":
                ...
                self._send_colors()
                # self._set_color_url(self._color_current[0], self._urls[2])
                # self._set_color_url(self._color_current[2], self._urls[1])
                # self._set_color_url(self._color_current[1], self._urls[0])
    ...
    def _send_colors(self):
        ...
        future_one = session.post("http://" + self._urls[2] + ":" + str(self._port) + "/save_settings",
            json={"led.brightness": self._brightness, "led.color": self._color_current[0]})
        future_two = session.post("http://" + self._urls[1] + ":" + str(self._port) + "/save_settings",
            json={"led.brightness": self._brightness, "led.color": self._color_current[2]})
        future_three = session.post("http://" + self._urls[0] + ":" + str(self._port) + "/save_settings",
            json={"led.brightness": self._brightness, "led.color": self._color_current[1]})
        self._settings.log.msg(str(future_one.result()))
        future_two.result()
        future_three.result()