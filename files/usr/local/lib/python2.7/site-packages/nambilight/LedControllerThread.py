...
class LedControllerThread(Thread):
    ...
    def _set_color(self, value=None):
        ...
        self._settings.log.msg("LedControllerThread: urls: " + str(self._urls))
        for url in self._urls:
            try:
                r = requests.post(url + ":" + str(self._port) + "/save_settings",
                                json={"led.brightness": 1, "led.color": value}, timeout=0.1)
            except requests.exceptions.ConnectionError as exception:
