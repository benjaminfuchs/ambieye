...
class ScreenThread(Thread):
    ...
    def run(self):
        ...
        self._settings.set("screen.ready", "false")
        self._settings.log.msg("ScreenThread: searching screen...")
        while not self._stop_event.is_set():
            if self._settings.get('screen.ready') == "false":
                try:
                    self.loop()
                    if len(self._x) == self.DEQUE_LENGHT:
                        ...
                        self._settings.set("screen.coordinates", (sum(self._x)/len(self._x), sum(self._y)/len(
                            self._y), sum(self._w)/len(self._w), sum(self._h)/len(self._h)))
                        self._settings.log.msg("ScreenThread: screen ready")
                        self._settings.set("screen.ready", "true")
                ...
                self._w = collections.deque(maxlen=self.DEQUE_LENGHT)
                self._h = collections.deque(maxlen=self.DEQUE_LENGHT)
                self._settings.log.msg("ScreenThread: running ScreenThread")
